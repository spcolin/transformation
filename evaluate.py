...
@torch.no_grad()
def validate_chairs(model, args,iters=24):
    """ Perform evaluation on the FlyingChairs (test) split """
    model.eval()
    epe_list = []

    val_dataset = datasets.FlyingChairs(split='validation',root=args.data_root+"FlyingChairs_release/data")
...
def validate_sintel(model, args,iters=32):
    ...
    results = {}
    for dstype in ['clean', 'final']:
        val_dataset = datasets.MpiSintel(split='training',root=args.data_root+"Sintel",  dstype=dstype)
        epe_list = []
...
def validate_kitti(model,args, iters=24):
    ...
    model.eval()
    val_dataset = datasets.KITTI(split='training',root=args.data_root+"KITTI")
